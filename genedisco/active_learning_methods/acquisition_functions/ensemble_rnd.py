...
class RND(BaseBatchAcquisitionFunction):
    def initialize(self,input_size):
       ...
       self.RNDS = torch.nn.ModuleList([initialize_model(input_size) for _ in range(4)])

       self.optimizers = []
       for model in self.RNDS:
        self.optimizers.append(optim.Adam(model.parameters(), lr=0.01))
    ...
    def __call__(self, dataset_x: AbstractDataSource, batch_size: int, available_indices: List[AnyStr],
        ...
        with torch.no_grad():
            outputs = [model(available_data) for model in self.RNDS]
            scores = torch.stack(outputs).var(0).mean(-1).numpy()
        ...
        i = 0
        for model, optimizer in zip(self.RNDS, self.optimizers):
            print(f"TRAINING MODEL {i}")
            self.train_on_points_to_acquire(model, optimizer, selected_indices, dataset_x)
            i += 1
    ...
    def train_on_points_to_acquire(self, model, optimizer, points_to_score, dataset_x):
        """
        ...
        for epoch in range(EPOCH):
            ...
            optimizer.step()
        model.eval() # set to non-train mode.